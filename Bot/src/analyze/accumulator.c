...
void extractInputLayerToArray(uint64_t* inputLayerCompact_w, uint64_t* inputLayerCompact_b, void* output, int outputType)
{
    ...
    if(outputType == TRAINING)
    {
        ...
        for(int i = 0; i < BITBOARDS_PER_INPUT_SIDE; i++)
        {
            ...
            if((inputLayerCompact_b[ (int) i / 64 ] >> ( i % 64 )) & 1) inputLayerFloats[BITBOARDS_PER_INPUT_SIDE + i] = 1.0;
            else inputLayerFloats[BITBOARDS_PER_INPUT_SIDE + i] = 0.0;
        }
    }
    else if(outputType == PLAYING)
    {
        ...
        for(int i = 0; i < BITBOARDS_PER_INPUT_SIDE; i++)
        {
            ...
            if((inputLayerCompact_b[ (int) i / 64 ] >> ( i % 64 )) & 1) inputLayerBytes[BITBOARDS_PER_INPUT_SIDE + i] = 1;
            else inputLayerBytes[BITBOARDS_PER_INPUT_SIDE + i] = 0;
        }
    }
}
...
void loadInputAccumulator(bitboard* board, void* accumulator, int accumulatorType, int color)
{
    ...
    memset(inputs, 0, 640*sizeof(uint64_t));
    ...
    if(byteAccumulator)
    {
        ...
        if(ISWHITE(color)) 
        {
            calculateLayer_IntBytes(inputArray, byteAccumulator->rawAccumulator[0], BITBOARDS_PER_INPUT_SIDE, ACCUMULATOR_NODES_PER_SIDE, playerNNUE->weights1, extendedBaseIndex_w, playerNNUE->weights1_bias, 0);
        ...
        if(ISBLACK(color)) 
        {
            calculateLayer_IntBytes(&inputArray[BITBOARDS_PER_INPUT_SIDE], byteAccumulator->rawAccumulator[1], BITBOARDS_PER_INPUT_SIDE, ACCUMULATOR_NODES_PER_SIDE, playerNNUE->weights1, extendedBaseIndex_b, playerNNUE->weights1_bias, 0);
    ...
    else
    {
        ...
        if(ISWHITE(color)) 
        {
            calculateLayer_Floats(inputArray, floatAccumulator->rawAccumulator[0], BITBOARDS_PER_INPUT_SIDE, ACCUMULATOR_NODES_PER_SIDE, trainingNNUE->weights1, extendedBaseIndex_w, trainingNNUE->weights1_bias, 0);
        ...
        if(ISBLACK(color)) 
        {
            calculateLayer_Floats(&inputArray[BITBOARDS_PER_INPUT_SIDE], floatAccumulator->rawAccumulator[1], BITBOARDS_PER_INPUT_SIDE, ACCUMULATOR_NODES_PER_SIDE, trainingNNUE->weights1, extendedBaseIndex_b, trainingNNUE->weights1_bias, 0);
...
void updateBoardAccumulator(bitboard* currentBoard, bitboard* accumulatorBoard, void* accumulator, int accumulatorType, int color)
{
    ...
    for(int piece = 0; piece < 10; piece++)
    {
        ...
        //Input node updates
        int ksq = ISWHITE(color) ? kingBuckets[accumulatorBoard->kingSquare_w] : kingBuckets[accumulatorBoard->kingSquare_b];
        ...
        while(difference)
        {
            ...
            int featureIndex_Black = (64 * (10 * kingBuckets[accumulatorBoard->kingSquare_b] + piece)) + square;
            int featureIndex_White = (64 * (10 * kingBuckets[accumulatorBoard->kingSquare_w] + piece)) + square;
